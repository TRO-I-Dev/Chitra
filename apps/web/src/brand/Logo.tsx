...
/**
 * Tsecond parent-brand attribution mark. Renders the WebP logo from the
 * public folder with an `onError` fallback that hides the image if the
 * asset is missing (e.g. on a fresh checkout where the user has not yet
 * copied `tsecond-logo.webp` into `apps/web/public/`).
 *
 * Used on the Welcome screen and Onboarding splash to acknowledge the
 * parent brand without competing with the Chitra product mark.
 */
export function TsecondMark({
  size = 22,
  withWordmark = true,
  className,
  caption,
  ...
  return (
    <div
      className={[
        "flex items-center gap-2 text-[var(--color-ink-dim)]",
        className ?? "",
      ].join(" ")}
    >
      <img
        src="./tsecond-logo.webp"
        alt="Tsecond"
        width={size}
        height={size}
        loading="lazy"
