...
export function Settings({
    ...
    void (async () => {
      try {
        const [settings, notionToken, confluenceToken] = await Promise.all([
          window.chitra.settingsGet(),
          window.chitra.secretGet({ key: "notion-token" }),
          window.chitra.secretGet({ key: "confluence-token" }),
        ]);
        if (cancelled) return;
        setForm({
  ...
  async function save(): Promise<void> {
    ...
    setError(null);
    try {
      await window.chitra.settingsSet({
      ...
      const setOrClear = async (key: SecretKey, value: string): Promise<void> => {
        if (value.trim()) await window.chitra.secretSet({ key, value: value.trim() });
        else await window.chitra.secretDelete({ key });
      };
            ...
            <header className="mb-5 flex items-baseline justify-between">
              <div>
                ...
                <p className="text-xs text-[var(--color-ink-dim)]">
                  Tokens are stored in your OS keychain. Other fields live in a JSON file in your user data folder.
                </p>
              </div>
              <button
                type="button"
